...
/**
 * Migration API Endpoint: Firestore to Supabase
 * 
 * POST /api/migrate/firestore
...
export async function POST(request: NextRequest) {
  ...
  try {
    const migrationModule = await import('../../../../scripts/migrate-firestore-to-supabase');
    const result = await migrationModule.default();
    
    return NextResponse.json({
      success: true,
      message: 'Firestore to Supabase migration completed successfully',
      result,
    });
  } catch (error: any) {
